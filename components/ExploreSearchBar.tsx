...
function CustomSearchContent() {
  ...
  return (
    <div className="fixed top-4 left-0 z-30 w-full h-fit px-4 mb-6">
      ...
      {isOpen && query.length > 0 && (
        <div className="absolute touch-none top-full left-4 right-4 mt-2 bg-white rounded-3xl shadow-2xl shadow-black/10 border-2 border-gray-100 z-50 overflow-hidden animate-in fade-in slide-in-from-top-2 duration-200">
          <div className="p-2">
            {items.length > 0 ? (
              items.slice(0, 5).map((hit: any) => (
                <div 
                  key={hit.objectID} 
                  ...
                  <div className="flex items-center space-x-4">
                    ...
                    <div>
                      ...
                      <p className="text-xs text-gray-400 capitalize">
                        {hit.vendor} • {hit.category || 'New Arrivals'}
                      </p>
                    </div>
                  </div>
            ...
            <div className="border-t border-gray-50 mt-1 pt-1">
              <div className="flex items-center p-3 hover:bg-gray-50 rounded-2xl cursor-pointer text-gray-700 group transition-colors">
      ...
      {isOpen && (
        <div className="fixed inset-0 touch-none z-40" onClick={() => setIsOpen(false)} />
      )}
    </div>
  );
}