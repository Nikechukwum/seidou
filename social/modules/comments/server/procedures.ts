...
export const commentsRouter = createTRPCRouter({
  create: protectedProcedure
    ...
    .mutation(async ({ input, ctx }) => {
      ...
      // Only one level of nesting: you cannot reply to a reply.
  ...
  getMany: baseProcedure
    ...
    .query(async ({ input, ctx }) => {
      ...
      const [totalData, data] = await Promise.all([
        // Counts every comment on the video, replies included — this is the
        // number shown as the section heading.
        db
          .select({ count: count() })
          .from(comments)
        ...
        db
          ...
          .where(
            and(
              ...
              // No parentId means top-level comments; passing one fetches
              // that comment's replies.
              parentId
