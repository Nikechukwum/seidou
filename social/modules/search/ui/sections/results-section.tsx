...
export const ResultsSection = ({ query, categoryId }: ResultsSectionProps) => {
  return (
    ...
    >
      {/* key: a new query must remount and re-suspend, otherwise the previous
          results stay on screen while the new ones load. */}
  ...
  if (items.length === 0) {
    return (
      <p className="px-4 py-12 text-center text-sm text-muted-foreground">
        {query ? `No results for "${query}".` : "No videos found."}
      </p>
    );
  }
